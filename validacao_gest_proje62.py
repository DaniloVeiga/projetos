...
COLUNAS_DEVOLUTIVA = [
    ...
    'Início_de_Programacao_Disposicao_DEVOLUTIVA',
...
COLUNAS_DROPDOWN_SIM_NAO = ['NOVO CATÁLOGO','TEVON','KOM_DEVOLUTIVA','Inicio_de_Compras_DEVOLUTIVA','Fim de Liberação - DEVOLUTIVA','Fim_de_Compras_DEVOLUTIVA','Início_de_Programacao_Disposicao_DEVOLUTIVA','Fim_da_Programacao_Disposicao_DEVOLUTIVA','Inicio_da_Liberacao_G0',
                            ...
                            'Fim_de_Liberacao_DEVOLUTIVA_G2','Fim_de_Liberacao_DEVOLUTIVA_G3','Fim_de_Liberacao_DEVOLUTIVA_G4','Fim_de_Liberacao_DEVOLUTIVA_G5','Fim_de_Liberacao_DEVOLUTIVA_G6','Fim_de_Liberacao_DEVOLUTIVA_G7','Fim_de_Liberacao_DEVOLUTIVA_G8','Fim_de_Liberacao_DEVOLUTIVA_G9',
                            'Inicio_de_Compras_DEVOLUTIVA', 'Fim_de_Compras_DEVOLUTIVA'] #Colunas com valores tipo SIM/NÃO.
...
def carregar_df(sheet):
    ...
    if 'Inicio_de_Compras' in df.columns:
        df['Inicio_compras_ISO'] = (
            pd.to_datetime(df['Inicio_de_Compras'], dayfirst=True, errors='coerce')
            .dt.strftime('%Y-%m-%d')
        )
    if 'Fim_de_Compras' in df.columns:
        df['Fim_de_Compras_ISO'] = (
            pd.to_datetime(df['Fim_de_Compras'], dayfirst=True, errors='coerce')
            .dt.strftime('%Y-%m-%d')
        )
    if 'Fim_de_Liberacao' in df.columns:
        df['Fim_Liberacao_ISO'] = (
            pd.to_datetime(df['Fim_de_Liberacao'], dayfirst=True, errors='coerce')
            .dt.strftime('%Y-%m-%d')
        )
    if 'Início_de_Programacao_Disposicao' in df.columns:
        df['Início_Prog_Dis_ISO'] = (
            pd.to_datetime(df['Início_de_Programacao_Disposicao'], dayfirst=True, errors='coerce')
            .dt.strftime('%Y-%m-%d')
        )
    if 'Fim_da_Programacao_Disposicao' in df.columns:
        df['Fim_Prog_Dis_ISO'] = (
            pd.to_datetime(df['Fim_da_Programacao_Disposicao'], dayfirst=True, errors='coerce')
            .dt.strftime('%Y-%m-%d')
        )
...
def criar_gantt(df, fases=None):
        ...
        def add_fase(ini, fim, fase):
            ...
            if pd.notna(ini) and pd.notna(fim):
                tarefas.append({
                    ...
                    'Linha': f'{programa} - {projeto}',
                    'Fase': fase,
                    'Início': ini,
                    'Fim': fim
                })

        add_fase(row['inicio_da_liberacao'], row['Fim_de_Liberacao'], 'Liberação')
        add_fase(row['Inicio_de_Compras'], row['Fim_de_Compras'], 'Compras')
        add_fase(
            row['Início_de_Programacao_Disposicao'],
            row['Fim_da_Programacao_Disposicao'],
    ...
    fig.update_xaxes(
        ...
        # ✅ NOVO - intervalo de 15 dias
        #🧠 EXPLICAÇÃO RÁPIDA
        #Plotly trabalha com tempo em milissegundos
        #1 dia = 24 * 60 * 60 * 1000
...
def carregar_cards_db():
    conn = sqlite3.connect(r".\base_sql\cards.db")
...
def carregar_cards_db_programas():
    conn = sqlite3.connect(r".\base_sql\cards.db")
...
def card_resumo(titulo, valor, cor):
    ...
    return html.Div(
        style={
            ...
            'justifyContent': 'flex-start',  # 👈 sobe o conteúdo
            'alignItems': 'center'

        },
        children=[
            
        html.H4(titulo, style={'margin': '0'}),
        html.Br(),
        html.H1(valor, style={'margin': '0'})
...
def aplicar_devolutivas(df_excel, df_devolutivas):
    ...
    # ✅ remove lixo gerado
    df_excel["Inicio_da_Liberacao_DEVOLUTIVA"] = df_excel["Inicio_da_Liberacao_DEVOLUTIVA"].replace(
        ["NONE", "NAN"], None
    )
    
    # ✅ REGERA DATA NORMAL (garante base limpa)
    df_excel['inicio_da_liberacao'] = (
        pd.to_datetime(df_excel['inicio_da_liberacao'], dayfirst=True, errors='coerce')
        .dt.strftime('%d/%m/%Y')
    )

    # ✅ REGRA FINAL
    mask_feito = df_excel["Inicio_da_Liberacao_DEVOLUTIVA"] == "FEITO"

    df_excel.loc[mask_feito, "inicio_da_liberacao"] = "FEITO"

    #___________________________________________________________________

    # ✅ remove lixo gerado
    df_excel["Fim_de_Liberacao_DEVOLUTIVA"] = df_excel["Fim_de_Liberacao_DEVOLUTIVA"].replace(
        ["NONE", "NAN"], None
    )
    
    # ✅ REGERA DATA NORMAL (garante base limpa)
    df_excel['Fim_de_Liberacao'] = (
        pd.to_datetime(df_excel['Fim_de_Liberacao'], dayfirst=True, errors='coerce')
        .dt.strftime('%d/%m/%Y')
    )

    # ✅ REGRA FINAL
    mask_feito = df_excel["Fim_de_Liberacao_DEVOLUTIVA"] == "FEITO"

    df_excel.loc[mask_feito, "Fim_de_Liberacao"] = "FEITO"
    #___________________________________________________________________

    # ✅ remove lixo gerado
    df_excel["Inicio_de_Compras_DEVOLUTIVA"] = df_excel["Inicio_de_Compras_DEVOLUTIVA"].replace(
        ["NONE", "NAN"], None
    )
    
    # ✅ REGERA DATA NORMAL (garante base limpa)
    df_excel['Inicio_de_Compras'] = (
        pd.to_datetime(df_excel['Inicio_de_Compras'], dayfirst=True, errors='coerce')
        .dt.strftime('%d/%m/%Y')
    )

    # ✅ REGRA FINAL
    mask_feito = df_excel["Inicio_de_Compras_DEVOLUTIVA"] == "SIM"

    df_excel.loc[mask_feito, "Inicio_de_Compras"] = "FEITO"
    #___________________________________________________________________

    # ✅ limpa lixo
    df_excel["Fim_de_Compras_DEVOLUTIVA"] = df_excel["Fim_de_Compras_DEVOLUTIVA"].replace(
        ["NONE", "NAN"], None
    )

    # ✅ NORMALIZA (evita erro de comparação)
    df_excel["Fim_de_Compras_DEVOLUTIVA"] = df_excel["Fim_de_Compras_DEVOLUTIVA"].apply(
        lambda x: str(x).strip().upper() if pd.notna(x) else None
    )

    # ✅ 1. REGERA DATA BASE (SEMPRE)
    df_excel['Fim_de_Compras'] = (
        pd.to_datetime(df_excel['Fim_de_Compras'], dayfirst=True, errors='coerce')
        .dt.strftime('%d/%m/%Y')
    )

    # ✅ 2. APLICA REGRA
    mask_sim = df_excel["Fim_de_Compras_DEVOLUTIVA"] == "SIM"

    df_excel.loc[mask_sim, "Fim_de_Compras"] = "FEITO"
    #___________________________________________________________________

    # ✅ limpa lixo
    df_excel["Início_de_Programacao_Disposicao_DEVOLUTIVA"] = df_excel["Início_de_Programacao_Disposicao_DEVOLUTIVA"].replace(
        ["NONE", "NAN"], None
    )

    # ✅ NORMALIZA (evita erro de comparação)
    df_excel["Início_de_Programacao_Disposicao_DEVOLUTIVA"] = df_excel["Início_de_Programacao_Disposicao_DEVOLUTIVA"].apply(
        lambda x: str(x).strip().upper() if pd.notna(x) else None
    )

    # ✅ 1. REGERA DATA BASE (SEMPRE)
    df_excel['Início_de_Programacao_Disposicao'] = (
        pd.to_datetime(df_excel['Início_de_Programacao_Disposicao'], dayfirst=True, errors='coerce')
        .dt.strftime('%d/%m/%Y')
    )

    # ✅ 2. APLICA REGRA
    mask_sim = df_excel["Início_de_Programacao_Disposicao_DEVOLUTIVA"] == "SIM"

    df_excel.loc[mask_sim, "Início_de_Programacao_Disposicao"] = "FEITO"
    #___________________________________________________________________

    # ✅ limpa lixo
    df_excel["Fim_da_Programacao_Disposicao_DEVOLUTIVA"] = df_excel["Fim_da_Programacao_Disposicao_DEVOLUTIVA"].replace(
        ["NONE", "NAN"], None
    )

    # ✅ NORMALIZA (evita erro de comparação)
    df_excel["Fim_da_Programacao_Disposicao_DEVOLUTIVA"] = df_excel["Fim_da_Programacao_Disposicao_DEVOLUTIVA"].apply(
        lambda x: str(x).strip().upper() if pd.notna(x) else None
    )

    # ✅ 1. REGERA DATA BASE (SEMPRE)
    df_excel['Fim_da_Programacao_Disposicao'] = (
        pd.to_datetime(df_excel['Fim_da_Programacao_Disposicao'], dayfirst=True, errors='coerce')
        .dt.strftime('%d/%m/%Y')
    )

    # ✅ 2. APLICA REGRA
    mask_sim = df_excel["Fim_da_Programacao_Disposicao_DEVOLUTIVA"] == "SIM"

    df_excel.loc[mask_sim, "Fim_da_Programacao_Disposicao"] = "FEITO"
...
def carregar_df_sqlite():
    ...
    df = pd.read_sql_query("""
        SELECT DATA_PROJ, PROGRAMA, PROJECT, PRODUCT, NOVO_CAT, NUM_CAT, FRONT_PAGE, TEVON,KOM, KOM_DEVOLUTIVA,
                           ...
                           , Inicio_da_Liberacao_G8, Inicio_da_Liberacao_G9, Fim_de_Liberacao, 
                           ...
                           Fim_de_Liberacao_DEVOLUTIVA, Inicio_de_Compras, Inicio_de_Compras_DEVOLUTIVA,Fim_de_Compras,
                           Fim_de_Compras_DEVOLUTIVA, Início_de_Programacao_Disposicao, Início_de_Programacao_Disposicao_DEVOLUTIVA,
                           Fim_da_Programacao_Disposicao,Fim_da_Programacao_Disposicao_DEVOLUTIVA, SOP, ME FROM tarefas
    """, conn)
    ...
    df.rename(columns={
        ...
        'Fim_de_Liberacao':'Fim_de_Liberacao',
        ...
        'inicio_da_liberacao': 'inicio_da_liberacao',
        'Fim_de_Liberacao': 'Fim_de_Liberacao',
        'Inicio_de_Compras': 'Inicio_de_Compras',
        'Fim_de_Compras': 'Fim_de_Compras',
        'Início_de_Programacao_Disposicao':'Início_de_Programacao_Disposicao',
        'Fim_da_Programacao_Disposicao':'Fim_da_Programacao_Disposicao',
        'COMMENTS': 'COMMENTS'
    }, inplace=True)
    ...
    if 'inicio_da_liberacao' in df.columns:
        ...
        data_dt = pd.to_datetime(df['inicio_da_liberacao'], dayfirst=True, errors='coerce')
        # ISO (comparação)
...
html.Div(
    ...
    children=[
        html.H1('Bem-vindo ao Sistema de Gestão de Projetos'),
        html.P('Acompanhe, valide e gerencie seus projetos de forma inteligente'),
...
html.Div(
    ...
    style={
        ...
        'gap': '30px',
        'position': 'relative'  # ✅ ESSENCIAL pro botão funcionar
    },
                    ...
                    dcc.Tabs([
                        ...
                        html.Br(),
                            ...
                            dash_table.DataTable(
                                ...
                                # ✅ ESCONDE AS ISO (AQUI!)
                                    hidden_columns=[c for c in df_controle.columns if c.endswith('_ISO')],
                                ...
                                **{
                                ...
                                style_data_conditional=[
                                    ...
                                    {
                                        'if': {
                                            'column_id': 'Fim_de_Compras',
                                            'filter_query': (
                                                f'{{Fim_de_Compras_ISO}} < "{HOJE_ISO}"'
                                            )
                                        },
                                        'backgroundColor': '#CD2626',
                                        'color': 'white',
                                    ...
                                    },
                                    {
                                        'if': {
                                            'column_id': 'Fim_de_Compras',
                                            'filter_query': (
                                                f'{{Fim_de_Compras_ISO}} > "{HOJE_MAIS_30}"'
                                    ...
                                    },
                                    {
                                        'if': {
                                            'column_id': 'Fim_de_Compras',
                                            'filter_query': (
                                                f'{{Fim_de_Compras_ISO}} > "{HOJE_MAIS_90}"'
                                    ...
                                    },
                                    {
                                        'if': {
                                            'column_id': 'Fim_de_Compras',
                                            'filter_query': (
                                                f'{{Fim_de_Compras_ISO}} > "{HOJE_MAIS_180}"'
                                            )
                                        },
                                        'backgroundColor': "#00FF2AC3",
                                        'color': 'black',
                                   ...
                                   #
                                    {
                                        'if': {
                                            'column_id': 'Início_de_Programacao_Disposicao',
                                            'filter_query': (
                                   ...
                                   #
                                    {
                                        'if': {
                                            'column_id': 'Fim_da_Programacao_Disposicao',
                                            'filter_query': (
                                    ...
                                    # 🟡 entre 180 e 365 dias
                                    {
                                        'if': {
                                            'column_id': 'SOP',
                                            'filter_query': (
                                                f'{{SOP_ISO}} > "{SOP_HOJE_MAIS_180}" && '
                                                f'{{SOP_ISO}} <= "{SOP_HOJE_MAIS_365}"'
                                            )
                                        },
                                        'backgroundColor': '#FFFF00',
                                        'color': 'black',
                                    ...
                                    },
                                    {
                                        'if': {
                                            'column_id': 'NOVO CATÁLOGO',
                                            'filter_query': '{NOVO CATÁLOGO} = "FEITO"'
                                        },
                                    ...
                                    },
                                    {
                                        'if': {
                                            'column_id': 'NÚMERO CATÁLOGO',
                                            'filter_query': '{NÚMERO CATÁLOGO} = "FEITO"'
                                        },
                                        'backgroundColor': '#CFCFCF',
                                        'color': 'black',
                                        'fontWeight': 'bold'
                                    ...
                                    },
                                    {
                                        'if': {
                                            ...
                                            'filter_query': '{Fim_de_Liberacao_DEVOLUTIVA} = "FEITO"'
                                        },
                                        'backgroundColor': '#00B050',
                                        'color': 'white',
                                        'fontWeight': 'bold'
                                    },
                                    {
                                        'if': {
                                            'column_id': 'Fim_de_Liberacao_DEVOLUTIVA',
                                            'filter_query': '{Fim_de_Liberacao_DEVOLUTIVA} = "PENDENTE"'
                                        },
                                        'backgroundColor': '#FF0000',
                                        'color': 'white',
                                        'fontWeight': 'bold'
                                    ...
                                    {
                                        'if': {
                                            'column_id': 'Inicio_de_Compras_DEVOLUTIVA',
                                            'filter_query': '{Inicio_de_Compras_DEVOLUTIVA} = "SIM"'
                                        },
                                        'backgroundColor': '#00B050',
                                        'color': 'white',
                                        'fontWeight': 'bold'
                                    },
                                    {
                                        'if': {
                                            'column_id': 'Inicio_de_Compras_DEVOLUTIVA',
                                            'filter_query': '{Inicio_de_Compras_DEVOLUTIVA} = "NÃO"'
                                        },
                                        'backgroundColor': '#FF0000',
                                        'color': 'white',
                                        'fontWeight': 'bold'
                                    ...
                                    {
                                        'if': {
                                            'column_id': 'Fim_de_Compras_DEVOLUTIVA',
                                            'filter_query': '{Fim_de_Compras_DEVOLUTIVA} = "SIM"'
                                        },
                                        'backgroundColor': '#00B050',
                                        'color': 'white',
                                        'fontWeight': 'bold'
                                    },
                                    {
                                        'if': {
                                            'column_id': 'Fim_de_Compras_DEVOLUTIVA',
                                            'filter_query': '{Fim_de_Compras_DEVOLUTIVA} = "NÃO"'
                                        },
                                        'backgroundColor': '#FF0000',
                                        'color': 'white',
                                        'fontWeight': 'bold'
                                    ...
                                    {
                                        'if': {
                                            'column_id': 'Início_de_Programacao_Disposicao_DEVOLUTIVA',
                                            'filter_query': '{Início_de_Programacao_Disposicao_DEVOLUTIVA} = "SIM"'
                                        },
                                        'backgroundColor': '#00B050',
                                        'color': 'white',
                                        'fontWeight': 'bold'
                                    },
                                    {
                                        'if': {
                                            'column_id': 'Início_de_Programacao_Disposicao_DEVOLUTIVA',
                                            'filter_query': '{Início_de_Programacao_Disposicao_DEVOLUTIVA} = "NÃO"'
                                        },
                                        'backgroundColor': '#FF0000',
                                        'color': 'white',
                                        'fontWeight': 'bold'
                                    ...
                                    {
                                        'if': {
                                            'column_id': 'Fim_da_Programacao_Disposicao_DEVOLUTIVA',
                                            'filter_query': '{Fim_da_Programacao_Disposicao_DEVOLUTIVA} = "SIM"'
                                        },
                                        'backgroundColor': '#00B050',
                                        'color': 'white',
                                        'fontWeight': 'bold'
                                    },
                                    {
                                        'if': {
                                            'column_id': 'Fim_da_Programacao_Disposicao_DEVOLUTIVA',
                                            'filter_query': '{Fim_da_Programacao_Disposicao_DEVOLUTIVA} = "NÃO"'
                                        },
                                        'backgroundColor': '#FF0000',
                                        'color': 'white',
                                        'fontWeight': 'bold'
                                    ...
                                    {
                                        'if': {
                                            'column_id': 'Fim_de_Liberacao_DEVOLUTIVA_G0',
                                            'filter_query': '{Fim_de_Liberacao_DEVOLUTIVA_G0} = "SIM"'
                                        },
                                        'backgroundColor': '#00B050',
                                        'color': 'white',
                                        'fontWeight': 'bold'
                                    },
                                    {
                                        'if': {
                                            'column_id': 'Fim_de_Liberacao_DEVOLUTIVA_G0',
                                            'filter_query': '{Fim_de_Liberacao_DEVOLUTIVA_G0} = "NÃO"'
                                        },
                                        'backgroundColor': '#FF0000',
                                        'color': 'white',
                                        'fontWeight': 'bold'
                                    ...
                                    {
                                        'if': {
                                            'column_id': 'Fim_de_Liberacao_DEVOLUTIVA_G1',
                                            'filter_query': '{Fim_de_Liberacao_DEVOLUTIVA_G1} = "SIM"'
                                        },
                                        'backgroundColor': '#00B050',
                                        'color': 'white',
                                        'fontWeight': 'bold'
                                    },
                                    {
                                        'if': {
                                            'column_id': 'Fim_de_Liberacao_DEVOLUTIVA_G1',
                                            'filter_query': '{Fim_de_Liberacao_DEVOLUTIVA_G1} = "NÃO"'
                                        },
                                        'backgroundColor': '#FF0000',
                                        'color': 'white',
                                        'fontWeight': 'bold'
                                    ...
                                    {
                                        'if': {
                                            'column_id': 'Fim_de_Liberacao_DEVOLUTIVA_G2',
                                            'filter_query': '{Fim_de_Liberacao_DEVOLUTIVA_G2} = "SIM"'
                                        },
                                        'backgroundColor': '#00B050',
                                        'color': 'white',
                                        'fontWeight': 'bold'
                                    },
                                    {
                                        'if': {
                                            'column_id': 'Fim_de_Liberacao_DEVOLUTIVA_G2',
                                            'filter_query': '{Fim_de_Liberacao_DEVOLUTIVA_G2} = "NÃO"'
                                        },
                                        'backgroundColor': '#FF0000',
                                        'color': 'white',
                                        'fontWeight': 'bold'
                                    ...
                                    {
                                        'if': {
                                            'column_id': 'Fim_de_Liberacao_DEVOLUTIVA_G3',
                                            'filter_query': '{Fim_de_Liberacao_DEVOLUTIVA_G3} = "SIM"'
                                        },
                                        'backgroundColor': '#00B050',
                                        'color': 'white',
                                        'fontWeight': 'bold'
                                    },
                                    {
                                        'if': {
                                            'column_id': 'Fim_de_Liberacao_DEVOLUTIVA_G3',
                                            'filter_query': '{Fim_de_Liberacao_DEVOLUTIVA_G3} = "NÃO"'
                                        },
                                        'backgroundColor': '#FF0000',
                                        'color': 'white',
                                        'fontWeight': 'bold'
                                    ...
                                    {
                                        'if': {
                                            'column_id': 'Fim_de_Liberacao_DEVOLUTIVA_G4',
                                            'filter_query': '{Fim_de_Liberacao_DEVOLUTIVA_G4} = "SIM"'
                                        },
                                        'backgroundColor': '#00B050',
                                        'color': 'white',
                                        'fontWeight': 'bold'
                                    },
                                    {
                                        'if': {
                                            'column_id': 'Fim_de_Liberacao_DEVOLUTIVA_G4',
                                            'filter_query': '{Fim_de_Liberacao_DEVOLUTIVA_G4} = "NÃO"'
                                        },
                                        'backgroundColor': '#FF0000',
                                        'color': 'white',
                                        'fontWeight': 'bold'
                                    ...
                                    {
                                        'if': {
                                            'column_id': 'Fim_de_Liberacao_DEVOLUTIVA_G5',
                                            'filter_query': '{Fim_de_Liberacao_DEVOLUTIVA_G5} = "SIM"'
                                        },
                                        'backgroundColor': '#00B050',
                                        'color': 'white',
                                        'fontWeight': 'bold'
                                    },
                                    {
                                        'if': {
                                            'column_id': 'Fim_de_Liberacao_DEVOLUTIVA_G5',
                                            'filter_query': '{Fim_de_Liberacao_DEVOLUTIVA_G5} = "NÃO"'
                                        },
                                        'backgroundColor': '#FF0000',
                                        'color': 'white',
                                        'fontWeight': 'bold'
                                    ...
                                    {
                                        'if': {
                                            'column_id': 'Fim_de_Liberacao_DEVOLUTIVA_G6',
                                            'filter_query': '{Fim_de_Liberacao_DEVOLUTIVA_G6} = "SIM"'
                                        },
                                        'backgroundColor': '#00B050',
                                        'color': 'white',
                                        'fontWeight': 'bold'
                                    },
                                    {
                                        'if': {
                                            'column_id': 'Fim_de_Liberacao_DEVOLUTIVA_G6',
                                            'filter_query': '{Fim_de_Liberacao_DEVOLUTIVA_G6} = "NÃO"'
                                        },
                                        'backgroundColor': '#FF0000',
                                        'color': 'white',
                                        'fontWeight': 'bold'
                                    ...
                                    {
                                        'if': {
                                            'column_id': 'Fim_de_Liberacao_DEVOLUTIVA_G7',
                                            'filter_query': '{Fim_de_Liberacao_DEVOLUTIVA_G7} = "SIM"'
                                        },
                                        'backgroundColor': '#00B050',
                                        'color': 'white',
                                        'fontWeight': 'bold'
                                    },
                                    {
                                        'if': {
                                            'column_id': 'Fim_de_Liberacao_DEVOLUTIVA_G7',
                                            'filter_query': '{Fim_de_Liberacao_DEVOLUTIVA_G7} = "NÃO"'
                                        },
                                        'backgroundColor': '#FF0000',
                                        'color': 'white',
                                        'fontWeight': 'bold'
                                    ...
                                    {
                                        'if': {
                                            'column_id': 'Fim_de_Liberacao_DEVOLUTIVA_G8',
                                            'filter_query': '{Fim_de_Liberacao_DEVOLUTIVA_G8} = "SIM"'
                                        },
                                        'backgroundColor': '#00B050',
                                        'color': 'white',
                                        'fontWeight': 'bold'
                                    },
                                    {
                                        'if': {
                                            'column_id': 'Fim_de_Liberacao_DEVOLUTIVA_G8',
                                            'filter_query': '{Fim_de_Liberacao_DEVOLUTIVA_G8} = "NÃO"'
                                        },
                                        'backgroundColor': '#FF0000',
                                        'color': 'white',
                                        'fontWeight': 'bold'
                                    ...
                                    {
                                        'if': {
                                            'column_id': 'Fim_de_Liberacao_DEVOLUTIVA_G9',
                                            'filter_query': '{Fim_de_Liberacao_DEVOLUTIVA_G9} = "SIM"'
                                        },
                                        'backgroundColor': '#00B050',
                                        'color': 'white',
                                        'fontWeight': 'bold'
                                    ...
                                    },
                                    {
                                        'if': {
                                            'column_id': 'inicio_da_liberacao',
                                            'filter_query': '{inicio_da_liberacao} = "FEITO"'
                                        },
                                        'backgroundColor': '#CFCFCF',  # cinza claro
                                        'color': 'black',
                                        'fontWeight': 'bold'
                                    },
                                    {
                                        'if': {
                                            'column_id': 'Fim_de_Liberacao',
                                            'filter_query': '{Fim_de_Liberacao} = "FEITO"'
                                        },
                                        'backgroundColor': '#CFCFCF',  # cinza claro
                                        'color': 'black',
                                        'fontWeight': 'bold'
                                    },
                                    {
                                        'if': {
                                            'column_id': 'Inicio_de_Compras',
                                            'filter_query': '{Inicio_de_Compras} = "FEITO"'
                                        },
                                        'backgroundColor': '#CFCFCF',  # cinza claro
                                        'color': 'black',
                                        'fontWeight': 'bold'
                                    },
                                    {
                                        'if': {
                                            'column_id': 'Fim_de_Compras',
                                            'filter_query': '{Fim_de_Compras} = "FEITO"'
                                        },
                                        'backgroundColor': '#CFCFCF',  # cinza claro
                                        'color': 'black',
                                        'fontWeight': 'bold'
                                    },
                                    {
                                        'if': {
                                            'column_id': 'Início_de_Programacao_Disposicao',
                                            'filter_query': '{Início_de_Programacao_Disposicao} = "FEITO"'
                                        },
                                        'backgroundColor': '#CFCFCF',  # cinza claro
                                        'color': 'black',
                                        'fontWeight': 'bold'
                                    ...
                                    ,
                                    {
                                        'if': {
                                            'column_id': 'Fim_da_Programacao_Disposicao',
                                            'filter_query': '{Fim_da_Programacao_Disposicao} = "FEITO"'
                                        },
                                        'backgroundColor': '#CFCFCF',  # cinza claro
                                        'color': 'black',
                                        'fontWeight': 'bold'
                        ...
                        dcc.Tab(label='Cronograma', children=[
                            ...
                            #✅ ✅ Como aproximar os botões
                            #🔹 Opção 1 (RECOMENDADA – simples e limpa)
                            html.Div(
                            style={
                                'display': 'flex',
                            ...
                            children=[
                                html.Button('Aplicar filtros', id='btn-aplicar-filtros', style={'backgroundColor': '#27AE60','color': 'white','border': 'none','padding': '6px 12px'}),
                                html.Button('Limpar filtros', id='btn-limpar-filtros', style={'backgroundColor': '#27AE60','color': 'white','border': 'none','padding': '6px 12px'}),
                                html.Button('Exportar PNG', id='btn-exportar-png', style={'backgroundColor': '#27AE60','color': 'white','border': 'none','padding': '6px 12px'}),
                                html.Button('Exportar PDF', id='btn-exportar-pdf', style={'backgroundColor': '#27AE60','color': 'white','border': 'none','padding': '6px 12px'}),
                        ...
                        # ✅ ✅ ESSA LINHA É A QUE FALTOU
                        ]),  # 🔥 FECHA O dcc.Tab(label='Cronograma', children=[ ... ])
                        # ===================== OVERVIEW =====================
                            ...
                            html.Div(
                                ...
                                children=[
                                    ...
                                    html.Button(
                                                ...
                                                dcc.Download(id='download-log-xlsx')  # ✅ ESSENCIAL
...
def controlar_colunas_por_setor(usuario):
   ...
   # ✅ VISUALIZAÇÃO TOTAL (visitante ou público)
    if setor == 'visualizacao':
        ...
        {
            'name': 'Inicio_de_Compras',
            'id': 'Inicio_de_Compras',
            'presentation': 'input',
            'editable': False
        },
        {
            'name': 'Inicio_de_Compras_DEVOLUTIVA',
            'id': 'Inicio_de_Compras_DEVOLUTIVA',
            'presentation': 'dropdown',
            'editable': False
        },
        {
        ...
        {
            'name': 'Fim_de_Compras_DEVOLUTIVA',
            'id': 'Fim_de_Compras_DEVOLUTIVA',
            'presentation': 'dropdown',
            'editable': False
        },
        {
            'name': 'Fim_de_Liberacao',
            'id': 'Fim_de_Liberacao',
            'presentation': 'input',
            'editable': False
        },
        ...
        #programação 
        {
            'name': 'Início_de_Programacao_Disposicao',
            'id': 'Início_de_Programacao_Disposicao',
            'presentation': 'input',
            'editable': False
        },
        {
            'name': 'Início_de_Programacao_Disposicao_DEVOLUTIVA',
            'id': 'Início_de_Programacao_Disposicao_DEVOLUTIVA',
            'presentation': 'dropdown',
            'editable': False
        },
        {
            'name': 'Fim_da_Programacao_Disposicao',
            'id': 'Fim_da_Programacao_Disposicao',
            'presentation': 'input',
            'editable': False
        },
        {
            'name': 'Fim_da_Programacao_Disposicao_DEVOLUTIVA',
            'id': 'Fim_da_Programacao_Disposicao_DEVOLUTIVA',
            'presentation': 'dropdown',
            'editable': False
        },
        #------------------------
...
##################################################
        colunas = [
            ...
            {'name': 'PROGRAMA', 'id': 'PROGRAMA', 'editable': False},
            {'name': 'PROJECT', 'id': 'PROJECT', 'editable': False},
            {'name': 'PRODUCT', 'id': 'PRODUCT', 'editable': False},
            {'name': 'NOVO CATÁLOGO','id': 'NOVO CATÁLOGO','presentation': 'input','editable': False},
            {'name': 'NÚMERO CATÁLOGO','id': 'NÚMERO CATÁLOGO','presentation': 'input','editable': False},
            {'name': 'inicio_da_liberacao', 'id': 'inicio_da_liberacao', 'editable': True}
        ]
        ...
        if grupo_idx is not None:
            ...
            # ✅ ✅ AQUI entra o FIM padrão (posição correta)
            colunas.append({
                'name': 'Fim_de_Liberacao',
                'id': 'Fim_de_Liberacao',
                'editable': True
    ...
    if setor == 'gestão':
        ...
        for c in df_controle.columns
        #if not c.endswith('_ISO')  # ✅ REMOVE TODAS AS COLUNAS ISO
    ]

    colunas = []
    for c in df_controle.columns:
        # ✅ não mostra coluna ISO para ninguém
        if c.endswith('_ISO'):
            continue
...
@app.callback(
    ...
    Input('btn-logout', 'n_clicks'),  # ✅ NOVO

    State('boas-vindas-container', 'style'),
    State('login-container', 'style')
)
def trocar_tela(usuario, n_ir_login, n_voltar, n_logout, boas_style, login_style):

    trigger = ctx.triggered_id  # ✅ SEMPRE PRIMEIRA LINHA

    # ✅ LOGOUT → vai direto para login
    if trigger == 'btn-logout':
        boas_style['display'] = 'none'
        login_style['display'] = 'flex'
        return boas_style, login_style, {'display': 'none'}, '', '', '', '', ''

    # ✅ Tela inicial
    if not usuario and not n_ir_login:
        boas_style['display'] = 'flex'
        login_style['display'] = 'none'
        return boas_style, login_style, {'display': 'none'}, '', '', '', '', ''

    # ✅ Ir para login
    if trigger == 'btn-ir-login':
        boas_style['display'] = 'none'
        login_style['display'] = 'flex'
        return boas_style, login_style, {'display': 'none'}, '', '', '', '', ''

    # ✅ Voltar para boas-vindas
    if trigger == 'btn-voltar':
        boas_style['display'] = 'flex'
        login_style['display'] = 'none'
        return boas_style, login_style, {'display': 'none'}, '', '', '', '', ''

    # ✅ Dashboard (login feito)
    if usuario:
...
def salvar_devolutivas_sqlite(devolutivas, usuario, data_ref):
    ...
    for d in devolutivas:
        projeto = str(data_ref[d["linha"]]["PROJECT"]).strip()
        coluna = d["coluna"]
        valor = d["valor"]
        ...
        if coluna in [
            ...
            valor_calc = "FEITO" if valor == "SIM" else "PENDENTE"

            cur.execute(f"""
                INSERT INTO devolutivas
                (data_hora, email, nome, setor, projeto, coluna, valor, {coluna})
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                data_hora,
                usuario["email"],
            ...
            data_ref[d["linha"]][coluna] = valor_calc
        ...
        if (
            ...
            data_ref[d["linha"]]["Inicio_da_Liberacao_DEVOLUTIVA"] = "PENDENTE"
            data_ref[d["linha"]]["Fim_de_Liberacao_DEVOLUTIVA"] = "PENDENTE"
        ...
        cur.execute("""
            INSERT INTO devolutivas
            (data_hora, email, nome, setor, projeto, coluna, valor,
             Inicio_da_Liberacao_DEVOLUTIVA, Fim_de_Liberacao_DEVOLUTIVA)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            data_hora,
            usuario["email"],
    ...
    # ==========================================
    # ✅ UPDATE GLOBAL (AQUI É O SEGREDO)
    # ==========================================
    for projeto in projetos_afetados:

        # ✅ STATUS_PROJETO (CORRIGIDO MESMO)
        cur.execute("""
    UPDATE devolutivas
    SET status_projeto = (
        SELECT
            CASE
                ...
                -- ✅ REGRA NOVA (MULTI-SETOR COMPLETO)
                WHEN (
                    
                    -- TÉCNICA
                    (
                        SELECT COUNT(DISTINCT d1.coluna)
                        FROM devolutivas d1
                        WHERE d1.projeto = devolutivas.projeto
                        AND d1.coluna LIKE 'Inicio_da_Liberacao_G%'
                        AND d1.valor = 'SIM'
                    ) = 10
                    ...
                    -- COMPRAS
                    (
                        SELECT COUNT(DISTINCT d2.coluna)
                        FROM devolutivas d2
                ...
                WHEN EXISTS (
                    SELECT 1
                    FROM devolutivas f2
                    WHERE f2.projeto = devolutivas.projeto
                    AND f2.setor = 'gestão'
                )
                THEN 'FEITO'
                ...
                CASE
                    WHEN (
                        (
                            ...
                            FROM devolutivas d2
                            WHERE d2.projeto = devolutivas.projeto
                            AND d2.coluna LIKE 'Inicio_da_Liberacao_G%'
                            AND d2.valor = 'SIM'
                        ) = 10
                        ...
                        (
                            SELECT COUNT(DISTINCT d3.email)
                            FROM devolutivas d3
                            WHERE d3.projeto = devolutivas.projeto
                            AND d3.coluna LIKE 'Inicio_da_Liberacao_G%'
                            AND d3.valor = 'SIM'
                        ) >= 4
                    )
                    THEN 'FEITO'
                ...
                CASE
                    WHEN (
                        (
                            ...
                            FROM devolutivas d2
                            WHERE d2.projeto = devolutivas.projeto
                            AND d2.coluna LIKE 'Fim_de_Liberacao_DEVOLUTIVA_G%'
                            AND d2.valor = 'SIM'
                        ) = 10
...
@app.callback(
    ...
    Input('btn-aplicar-filtros', 'n_clicks'),  # ✅ botão manda executar
    State('filtro-programa', 'value'),
...
def atualizar_gantt(n_clicks, programas, projetos, fases, ini, fim):
    ...
    if ini and fim:
        ...
        df = df[
            ...
            (df['Fim_da_Programacao_Disposicao'] >= ini)
        ]
    ...
    fig = criar_gantt(df, fases)

    if ini and fim:
        fig.update_xaxes(range=[ini, fim])
...
@app.callback(
    ...
    State('grafico-gantt', 'figure'),
    prevent_initial_call=True
)
def exportar_pdf(n_clicks, fig):
    if not fig:
        raise PreventUpdate
    ...
    # recria figura
    fig_obj = pio.from_json(pio.to_json(fig))
...
@app.callback(
    ...
    Input('btn-exportar-png', 'n_clicks'),
    State('grafico-gantt', 'figure'),
    prevent_initial_call=True
)
def exportar_png(n_clicks, fig):
    if not fig:
        raise PreventUpdate
...
def atualizar_overview(_):

    df = carregar_cards_db()
    df_programas = carregar_cards_db_programas()
    ...
    df['SOP_DT'] = pd.to_datetime(df['SOP'], errors='coerce', dayfirst=True)

    # ✅ NORMALIZA TEXTO
    df['NOVO_CAT'] = df['NOVO_CAT'].astype(str).str.strip().str.upper()

    # --- SOP ---
    
    total_sop_2026 = df_programas['SOP'].str.contains('2026', na=False).sum()
    total_sop_2027 = df_programas['SOP'].str.contains('2027', na=False).sum()
    total_sop_2028 = df_programas['SOP'].str.contains('2028', na=False).sum()

    total_sim = df.loc[df['NOVO_CAT'] == 'SIM', 'NUM_CAT'].nunique()
    total_nao = df.loc[df['NOVO_CAT'] == 'NÃO', 'NUM_CAT'].nunique()
    total_definir = df.loc[df['NOVO_CAT'] == 'DEFINIR', 'NUM_CAT'].count()

    tempo_6m_resultado = df.loc[df['DATA_PROJ'] == '6 MESES', 'PROGRAMA'].nunique()
    tempo_post_resultado = df.loc[df['DATA_PROJ'] == 'POSTERIOR', 'PROGRAMA'].nunique()
    total_PROGRAMAS = int(tempo_6m_resultado+tempo_post_resultado)
    blank = ''

    return [
        card_resumo('[6 MESES]', tempo_6m_resultado, "#EA1010"),
        card_resumo('[POSTERIOR]', tempo_post_resultado, "#C2B912"),
        card_resumo('', blank, "#FFFFFF"),
        
        card_resumo('SOP (2026)', total_sop_2026, "#EA1010"),
        card_resumo('SOP (2027)', total_sop_2027, "#C2B912"),
        card_resumo('SOP (2028)', total_sop_2028, "#1FC836"),

        card_resumo('NOVO CATÁLOGO – [sim]', total_sim, "#EA1010"),
        card_resumo('NOVO CATÁLOGO – [não]', total_nao, "#C2B912"),
        card_resumo('NOVO CATÁLOGO – [definir]', total_definir, "#1FC836"),
...
def carregar_log_sqlite(n_clicks, usuario):
    ...
    query = """
        SELECT
            ...
            Inicio_de_Compras_DEVOLUTIVA,  -- ✅ NOVA COLUNA
            Início_de_Programacao_Disposicao_DEVOLUTIVA,  -- ✅ NOVA COLUNA
            Fim_de_Liberacao_DEVOLUTIVA,  -- ✅ NOVA COLUNA
            Fim_de_Compras_DEVOLUTIVA, -- ✅ NOVA COLUNA
            Fim_da_Programacao_Disposicao_DEVOLUTIVA,
            status_projeto
        FROM devolutivas
...
def ajustar_tabela_devolutivas():
    ...
    colunas_novas = [
        ...
        "Inicio_da_Liberacao_DEVOLUTIVA TEXT",
        "Fim_de_Liberacao_DEVOLUTIVA TEXT",
        "Inicio_de_Compras_DEVOLUTIVA TEXT",  # ✅ NOVA COLUNA
        "Fim_de_Compras_DEVOLUTIVA TEXT",
        "Início_de_Programacao_Disposicao_DEVOLUTIVA TEXT"
    ]
...
@app.callback(
    Output('download-log-xlsx', 'data'),
    Input('btn-exportar-log', 'n_clicks'),
    State('tabela-log', 'data'),
    prevent_initial_call=True
)
def exportar_log_excel(n_clicks, dados):
    if not dados:
        raise PreventUpdate
    ...
    nome_arquivo = f"log_devolutivas_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
