...
def criar_banco_cards():
    ...
    conn2 = sqlite3.connect(r".\base_sql\cards.db")
    cur2 = conn2.cursor()
...
def card():
    leitor = pd.read_excel('Project Summary - Volkswagen.xlsx', sheet_name='CARDS')
    leitor = leitor.drop(columns=leitor.columns[0])
    leitor.to_excel(r'.\base_convertida\cards.xlsx', index=False)
...
def validacao():
    ...
    colunas_data = ['Inicio_de_Compras']
    
    for col in colunas_data:
        if col in leitor.columns:
            leitor[col] = pd.to_datetime(leitor[col], dayfirst=True, errors='coerce')


    leitor.to_excel(r'.\base_validada\validacao_controle_TABELA.xlsx', index=False, header=True)

def validacao_card():
    leitor = pd.read_excel(r'.\base_convertida\cards.xlsx',skiprows=3)
    leitor.rename(columns={leitor.columns[0]:'DATA'}, inplace=True)
    colunas_data = ['Inicio_de_Compras']
    ...
    leitor.to_excel(r'.\base_validada\cards.xlsx', index=False, header=True)
...
def importador_demandas():
    leitor = pd.read_excel(r'.\base_validada\base_compilada_padrao.xlsx',sheet_name='Controle do Projeto')
...
def importador_cards_catalogo():
    leitor = pd.read_excel(r'.\base_validada\cards.xlsx')
    conta_linhas = len(leitor)
    # ---------- CONEXÃO ----------
    conexao = sqlite3.connect(r".\base_sql\cards.db")
    cursor = conexao.cursor()
...
def importador_cards_programas():
    leitor2 = pd.read_excel(r'.\base_validada\base_compilada_padrao.xlsx',sheet_name='Controle do Projeto')
    conta_linhas2 = len(leitor2)
    # ---------- CONEXÃO ----------
    conexao2 = sqlite3.connect(r".\base_sql\cards.db")
    ...
    for i in range(conta_linhas2):
        valores = (
            ...
            leitor2.iloc[i, 6],
            leitor2.iloc[i, 7],
            formatar_data(leitor2.iloc[i, 8]),
            leitor2.iloc[i, 9],
            formatar_data(leitor2.iloc[i, 10]),
            formatar_data(leitor2.iloc[i, 22]),
            formatar_data(leitor2.iloc[i, 24]),
            formatar_data(leitor2.iloc[i, 36]),
            formatar_data(leitor2.iloc[i, 38]),
            formatar_data(leitor2.iloc[i, 40]),
            formatar_data(leitor2.iloc[i, 42]),
            formatar_data(leitor2.iloc[i, 43]),
...
criar_banco_cards()
print('Criação de banco de dados cards - OK!')
paleta()
controle()
card()
flag()
...
importador_cards_programas()
print('Importação de base Cards_Programas - OK!')
importador_cards_catalogo()
print('Importação de base Cards_Catálogo - OK!')
print('{:.<100}'.format(''))